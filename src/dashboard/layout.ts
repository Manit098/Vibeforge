...
export const renderLayout = (
  ...
  const navHtml = navItems
    .map(
      (n) =>
        `<a class="nav-item${n.id === pageId ? ' active' : ''}" href="${n.href}" data-page="${n.id}">
...
<body>
  ...
  <script>
    ...
    function toggleTheme() {
      ...
      document.getElementById('theme-toggle').innerHTML = next === 'dark' ? '🌙 Dark' : '☀️ Light';
    }
    // Restore theme
    (function(){
      ...
      if (saved) {
        ...
        document.getElementById('theme-toggle').innerHTML = saved === 'dark' ? '🌙 Dark' : '☀️ Light';
      }
    })();
