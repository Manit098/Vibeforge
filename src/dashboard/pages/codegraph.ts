...
export const codegraphPage = (): string => {
  ...
  const script = `
    ...
    const fileIcons = {'.ts':'🔷','.js':'🟡','.json':'📋','.md':'📝','.css':'🎨','.html':'🌐','.txt':'📄','.yml':'⚙️','.yaml':'⚙️','.lock':'🔒','.gitignore':'🙈'};
    ...
    function renderTree(node, depth, parentPath) {
      ...
      if (node.type === 'directory' && node.children) {
        ...
        html += indent + '<span class="dir-arrow" id="arrow-'+id+'" style="font-size:0.7rem;color:var(--text-dim);width:1rem;text-align:center;">▼</span> ';
        html += icon + ' <span style="font-weight:600;color:var(--accent2);">'+escHtml(node.name)+'/</span>';
        ...
        html += indent + '&nbsp;'.repeat(4) + icon + ' <span style="color:var(--text);">'+escHtml(node.name)+'</span>';
    ...
    fetch('/api/codegraph').then(r=>r.json()).then(data=>{
      ...
      if (data.children) {
        ...
        document.getElementById('top-level').innerHTML = sorted.map(c=>{
          const icon = getIcon(c.name, c.type==='directory');
          const s = countTree(c);
          ...
          return '<div style="display:flex;align-items:center;gap:0.5rem;padding:0.35rem 0;border-bottom:1px solid var(--border);font-size:0.85rem;">'
            +icon+' <span style="font-weight:600;flex:1;">'+escHtml(c.name)+'</span>'
            +'<span style="color:var(--text-dim);font-size:0.75rem;">'+detail+'</span></div>';
        }).join('');
      }
    }).catch(()=>{ document.getElementById('tree-view').innerHTML='<div class="empty"><div class="empty-icon">❌</div>Failed to load</div>'; });
  `;
