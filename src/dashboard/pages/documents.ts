  ...
  const script = `
    ...
    function loadDocs() {
      ...
      fetch('/api/docs?category=' + cat).then(r=>r.json()).then(files=>{
        ...
        if(!files.length){ el.innerHTML='<div class="empty"><div class="empty-icon">📭</div>No files found</div>'; return; }
        el.innerHTML = files.map(f=>{
          const icon = f.category==='docs'?'📄':f.category==='memory'?'🧠':f.category==='records'?'📁':'📋';
