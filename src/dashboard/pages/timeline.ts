...
export const timelinePage = (): string => {
  const body = `
    ...
    <div class="timeline" id="timeline-feed">
      <div class="empty"><div class="empty-icon">⏳</div>Loading timeline...</div>
    </div>
  `;
  ...
  const script = `
    ...
    function renderItems(items) {
      ...
      if (!items.length) { el.innerHTML = '<div class="empty"><div class="empty-icon">📭</div>No matching entries</div>'; return; }
    ...
    function loadTimeline() {
      fetch('/api/timeline').then(r=>r.json()).then(items=>{
        ...
        document.getElementById('timeline-feed').innerHTML='<div class="empty"><div class="empty-icon">❌</div>Failed to load</div>';
      });
    }
    loadTimeline();
  `;
