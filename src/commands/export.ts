...
export const readDirRecursive = (dir: string, relativeRoot = ''): { [key: string]: string } => {
  ...
  for (const item of items) {
    ...
    const relativePath = path.join(relativeRoot, item.name);
    if (item.isDirectory()) {
      Object.assign(result, readDirRecursive(fullPath, relativePath));
    } else if (item.isFile()) {
      try {
        result[relativePath] = fs.readFileSync(fullPath, 'utf-8');
      } catch {
        result[relativePath] = '[Binary file skipped]';
      }
    }
  }
  return result;
