...
export const logCommand = (options: any) => {
  ...
  console.log(`${DIM}Showing ${Math.min(limit, items.length)} of ${items.length} entries${RESET}\n`);
  ...
  items.slice(0, limit).forEach((item) => {
    ...
    const nameShort = item.filename.length > 45 ? item.filename.substring(0, 42) + '...' : item.filename;
